...
int64_t AmountFromValue(const Value& value)
{
    ...
    if (dAmount <= 0.0 || dAmount > 2000000000.0)
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Gapcoin server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        string strWhatAmI = "To use gapcoind";
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("%s, you must set a rpcpassword in the configuration file:\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"Gapcoin Alert\" admin@foo.com\n"),
                strWhatAmI,
...
std::string HelpExampleCli(string methodname, string args){
    return "> gapcoin-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleRpc(string methodname, string args){
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:31397/\n";
}
